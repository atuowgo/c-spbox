...
flowchart TD
    ...
    G -- 是 --> I{有有效豁免中?}
    I -- 是 --> H
    I -- 否 --> M{紧急场景?<br/>来电/闹钟/导航}
    M -- 是 --> H
    M -- 否 --> N[全屏劝返遮罩<br/>+ 30 秒宽限]
    N --> O{30 秒内锁屏/<br/>停止/启用豁免?}
    O -- 是 --> H
    O -- 否 --> J[升级告警:<br/>持续震动 + 嗡鸣]
    J --> K{用户锁屏 / 启用豁免?}
    K -- 是 --> H
    K -- 否 --> J
    H -- 否 --> F
...
stateDiagram-v2
    ...
    Active --> Finished: 到达结束时间 / 离家
    Exempted --> Finished: 到达结束时间 / 离家
    Alarming --> Finished: 到达结束时间 / 离家
    Finished --> Idle
```
...
✅ **已确认（Q1）：三种方式全部支持，多信号融合。**

方案（Android，优先级从上到下）：
1. **家庭 WiFi SSID 检测**：连接到指定家庭 WiFi = 到家。低功耗、准确，作为主信号。
2. **地理围栏 Geofencing**：以家为中心设置围栏，进入围栏触发。作为无 WiFi 时的补充与交叉验证。
3. **手动确认到家**：提供"我到家了"手动入口作为兜底，任一自动信号漏判时用户可自行触发。
4. （可选，P1）蓝牙：连接到家里的某个蓝牙设备。

任一信号命中"从离家 → 到家"的状态翻转即触发（去抖动见下），手动确认可随时覆盖。
...
✅ **已确认（Q2）：采用「整机」口径，但放行电话等紧急场景。**
- 在自律期内点亮并解锁手机使用 = 违规。
- **例外放行**：处于**通话中/来电界面**等紧急场景不算违规、不告警（详见 5.5 放行清单）。
- 判断"是否在打电话"可通过 `TelephonyManager` 通话状态 + 前台为拨号/通话类应用来识别。

✅ **已确认（Q3）：宽限 30 秒。**
- 检测到违规使用后，先进入 **30 秒宽限**并弹出**全屏劝返遮罩**（提示"自律模式进行中，请放下手机 / 使用豁免"）。
- 30 秒内用户锁屏/停止使用/启用豁免 → 不告警。
- 超过 30 秒仍在使用且无豁免 → 升级为**持续震动 + 嗡鸣告警**。
...
- ✅ 已确认（Q4）：豁免**可提前手动结束**以节省剩余时长；剩余额度**每个自律周期清零**，不跨周期累计。
...
- **✅ 已确认（Q5）：放行清单，命中即不告警：**
  - **来电 / 通话中 / 紧急呼叫**（必放行，安全第一）
  - **闹钟**
  - **地图导航**
  - 放行判定：通话状态、前台应用属于放行类别、或系统正在响铃/导航中。
  - 放行清单在设置页可查看，紧急/来电为强制放行不可关闭。
- 音量、震动强度可调，并设"最大连续告警时长"上限以防意外（如单次最长 5 分钟自动降级）。

---

### 5.6 防作弊模块（Q7：中度）
**目标**：防止用户在自律期内一时冲动"反悔"，但不做操作系统级硬封锁。

✅ **已确认（Q7）：中度防作弊。**
- **自律期内以下操作受保护**，需**消耗一次豁免额度**或**输入自律密码**才能执行：
  - 修改当前自律参数（延迟/时长/豁免设置）
  - 提前结束/取消本次自律
  - 关闭监控/退出核心服务
- **自律密码**：在设置里预先设定（可选设置"设定后自己也无法在自律期内查看/重置"以增强约束）。
- 无障碍/悬浮窗等关键权限若被关闭，App 检测到后弹全屏提醒并尝试重新引导授予。
- 说明：Android 消费级 App 无法阻止用户彻底卸载或强制停止进程，此为已知边界（见 8.3）；
  "尽量强防"（设备管理员等）列为 P2，本版不做。
...
| 场景 | 处理策略 |
|------|----------|
| 自律期间离开家 | ✅ 已确认（Q6）：**离家自动结束**（断开家庭 WiFi / 出地理围栏即结束本次自律）|
...
| 编号 | 问题 | 结论 |
|------|------|--------------|
| **Q1** ✅ | 到家检测方式 | **已定：WiFi + 地理围栏 + 手动确认，三者都要** |
| **Q2** ✅ | 违规口径 | **已定：整机，但放行电话等紧急场景** |
| **Q3** ✅ | 告警触发时机 | **已定：宽限 30 秒（先全屏劝返，超时再升级告警）** |
| **Q5** ✅ | 告警放行清单 | **已定：放行来电/通话/紧急、闹钟、导航** |
| **Q4** ✅ | 豁免可否提前结束以省额度？额度是否每周期清零？ | **已定：可提前结束；每周期清零** |
| **Q6** ✅ | 自律期间离家如何处理？ | **已定：离家自动结束** |
| **Q7** ✅ | 防作弊强度 | **已定：中度（改参数/提前结束需消耗豁免或输密码）** |
| **Q8** ✅ | App 名称 | **已定：沿用代号 HomeLock，视觉待原型阶段** |
